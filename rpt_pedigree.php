...
function fillChart($gens, $ancestors, $topy, $yvals) {
    ...
    if ($gens <= 4) {
        ...
        if ($gens == 5) {
            ...
            // generation 2
            $x = $lftmrg + $boxspacing;
            $xwid = $xwid + $leftindex - $boxspacing;
            drawIndInfo($ancestors[1][0], $x, $yvals[3][0] - ($fathboxheight / 2), $xwid, 0);
            drawIndInfo($ancestors[1][1], $x, $yvals[3][1] - ($mothboxheight / 2), $xwid, 1);
...
function drawHusbWifeCombo($w, $num, $fh, $mh, $hsp, $vsp, $endspace = 0, $drawlines = 1, $listmode = 0, $boxmode = 0) {
    ...
    if ($drawlines) {
        $pdf->Line($x - ($hsp / 2), $y1, $x, $y1);
    }          // put a short line in the middle of the box
    $ytmp = $y + ($fh / 2);
    $y += $fh + $vsp;
    if ($boxmode == 0) {
    ...
    if ($drawlines) {
        $pdf->Line($x - ($hsp / 2), $y2, $x, $y2);
    }          // put a short line in the middle of the box
    if ($drawlines) {
        $pdf->Line($x - ($hsp / 2), $y1, $x - ($hsp / 2), $y2);
    }        // draw the vertical line between them
    if ($drawlines) {
        $pdf->Line($x - $hsp, ($y1 + $y2) / 2, $x - ($hsp / 2), ($y1 + $y2) / 2);
    }  // put a short line which will hook to the prev gen
    $y += $mh + $endspace;
    $pdf->SetY($y);
    $pdf->SetX($x);
