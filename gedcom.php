                ...
                if ($result) {
                    while ($spouse = tng_fetch_assoc($result)) {
                        if ($spouse['wife'] != $family['wife']) {
                            if (!array_key_exists($spouse['wife'], $indarray)) {
                                $indarray[$spouse['wife']] = writeIndividual($spouse['wife']);
                            }
                            if (!strstr($indarray[$spouse['wife']], "1 FAMS @{$spouse['familyID']}@")) {
                                $indarray[$spouse['wife']] .= "1 FAMS @{$spouse['familyID']}@$lineending";
                            }
                            $famarray[$spouse['familyID']] = writeFamily($spouse);
                        }
                        if (!strstr($indarray[$family['husband']], "1 FAMS @{$spouse['familyID']}@")) {
                ...
                if ($result) {
                    while ($spouse = tng_fetch_assoc($result)) {
                        if ($spouse['husband'] != $family['husband']) {
                            if (!array_key_exists($spouse['husband'], $indarray)) {
                                $indarray[$spouse['husband']] = writeIndividual($spouse['husband']);
                            }
                            if (!strstr($indarray[$spouse['husband']], "1 FAMS @{$spouse['familyID']}@")) {
                                $indarray[$spouse['husband']] .= "1 FAMS @{$spouse['familyID']}@$lineending";
                            }
                            $famarray[$spouse['familyID']] = writeFamily($spouse);
                        }
                        if (!strstr($indarray[$family['wife']], "1 FAMS @{$spouse['familyID']}@")) {
                ...
                if ($result) {
                    while ($child = tng_fetch_assoc($result)) {
                        if ($child['personID'] != $person) {
                            if (!array_key_exists($child['personID'], $indarray)) {
        ...
        if ($rights['both']) {
            ...
            while ($assoc = tng_fetch_assoc($assocresult)) {
                ...
                if ($assoc['relationship']) {
                    $info .= "2 RELA {$assoc['relationship']}$lineending";
                }
            }
    ...
    if ($result) {
        while ($family = tng_fetch_assoc($result)) {
            if (!$famarray[$family['familyID']]) {
                ...
                if (!array_key_exists($family[$spouse], $indarray)) {
                    $indarray[$family[$spouse]] = writeIndividual($family[$spouse]);
                }
                if (!strstr($indarray[$family[$spouse]], "1 FAMS @{$family['familyID']}@")) {
                    $indarray[$family[$spouse]] .= "1 FAMS @{$family['familyID']}@$lineending";
                }
                if (!strstr($indarray[$person], "1 FAMS @{$family['familyID']}@")) {
    ...
    if ($newsources) {
        foreach ($newsources as $nextsource) {
            ...
            if ($srcresult) {
                ...
                if ($source['author']) {
                    echo "1 AUTH {$source['author']}$lineending";
                }
                if ($source['publisher']) {
                    echo "1 PUBL {$source['publisher']}$lineending";
                }
                if ($source['repoID']) {
                    echo "1 REPO @{$source['repoID']}@$lineending";
                    array_push($allrepos, $source['repoID']);
                    if ($source['callnum']) {
                        echo "2 CALN {$source['callnum']}$lineending";
                    }
                } else {
                    if ($source['callnum']) {
                        echo "1 CALN {$source['callnum']}$lineending";
                    }
                }
