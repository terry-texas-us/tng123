...
var nicEditorConfig = bkClass.extend({
    buttons: {
        'bold': {name: __(nicbold), command: 'Bold', tags: ['B', 'STRONG'], css: {'font-weight': 'bold'}, key: 'b'},
        'italic': {name: __(nicitalic), command: 'Italic', tags: ['EM', 'I'], css: {'font-style': 'italic'}, key: 'i'},
        'underline': {name: __(nicunderline), command: 'Underline', tags: ['U'], css: {'text-decoration': 'underline'}, key: 'u'},
        'left': {name: __(niclalign), command: 'justifyleft', noActive: true},
        'center': {name: __(niccalign), command: 'justifycenter', noActive: true},
        'right': {name: __(nicralign), command: 'justifyright', noActive: true},
        'justify': {name: __(nicjalign), command: 'justifyfull', noActive: true},
        'ol': {name: __(nicinsol), command: 'insertorderedlist', tags: ['OL']},
        'ul': {name: __(nicinsul), command: 'insertunorderedlist', tags: ['UL']},
        'subscript': {name: __(nicsub), command: 'subscript', tags: ['SUB']},
        'superscript': {name: __(nicsup), command: 'superscript', tags: ['SUP']},
        'strikethrough': {name: __(nicstrike), command: 'strikeThrough', css: {'text-decoration': 'line-through'}},
        'removeformat': {name: __(nicremfmt), command: 'removeformat', noActive: true},
        'indent': {name: __(nicindent), command: 'indent', noActive: true},
        'outdent': {name: __(nicremind), command: 'outdent', noActive: true},
        'hr': {name: __(nichr), command: 'insertHorizontalRule', noActive: true}
    },
...
var nicEditor = bkClass.extend({
    construct: function (C) {
        ...
        var A = new bkElement("DIV").setStyle({width: (parseInt(B.getStyle("width")) + 90 || B.clientWidth + 90) + "px"}).appendBefore(B);
        this.setPanel(A);
...
var nicEditorInstance = bkClass.extend({
    isSelected: false, construct: function (G, D, C) {
        ...
        if (H || this.options.hasPanel) {
            ...
            var E = {width: parseInt(newX + 90) + "px", border: "1px solid #ccc", borderTop: 0, overflowY: "auto", overflowX: "hidden"};
            ...
            var A = new bkElement("DIV").setStyle({width: (newX + 82) + "px", padding: "4px", minHeight: newY + "px"}).addClass("main").appendTo(this.editorContain);
...
var nicEditorSelect = bkClass.extend({
    construct: function (D, A, C, B) {
        ...
        this.contain = new bkElement("div").setStyle({width: "120px", height: "20px", cursor: "pointer", overflow: "hidden"}).addClass("selectContain").addEvent("click", this.toggle.closure(this)).appendTo(this.margin);
        ...
        this.txt = new bkElement("div").setStyle({overflow: "hidden", "float": "left", width: "96px", height: "14px", marginTop: "3px", fontFamily: "sans-serif", fontSize: "12px"}).addClass("selectTxt").appendTo(this.items);
        if (!window.opera) {
...
var nicLinkOptions = {
    buttons: {
        'link': {name: niclink, type: 'nicLinkButton', tags: ['A']},
        'unlink': {name: nicremlink, command: 'unlink', noActive: true}
    }
};
...
var nicColorOptions = {
    buttons: {
        'forecolor': {name: __(nictxtcol), type: 'nicEditorColorButton', noClose: true},
        'bgcolor': {name: __(nicbkgcol), type: 'nicEditorBgColorButton', noClose: true}
    }
};
...
var nicImageOptions = {
    buttons: {
        'image': {name: nicimage, type: 'nicImageButton', tags: ['IMG']}
    }
...
var nicCodeOptions = {
    buttons: {
        'xhtml': {name: 'HTML', type: 'nicCodeButton'}
    }

};

var nicCodeButton = nicEditorAdvancedButton.extend({
    width: "850px", addPane: function () {
        this.addForm({"": {type: "title", txt: "HTML"}, code: {type: "content", value: this.ne.selectedInstance.getContent(), style: {width: "840px", height: "200px"}}})
    }, submit: function (B) {
