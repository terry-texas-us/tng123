        ...
        <table class="sntable">
            <tr>
                <td class="sncol">
                  ...
                  $query = "SELECT ucase( $binary $firstnamestr ) as firstname, $firstnamestr as lowername, ucase($binary firstname) as binlast, count( ucase($binary firstname) ) as lncount FROM $people_table WHERE ucase($binary TRIM(firstname)) LIKE \"$urlfirstchar%\" $wherestr2 GROUP BY lowername ORDER by binlast";
                  $result = tng_query($query);
