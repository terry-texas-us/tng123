                ...
                case "_SHAR":
                    ...
                    if (preg_match("/^@(\S+)@/", $lineinfo['rest'], $matches)) {
                        ...
                        while ($lineinfo['level'] >= $prevlevel + 1) {
                            ...
                            switch ($lineinfo['tag']) {
                                case "ROLE":
                                    $note['NOTE'] = _('Role') . ": " . addslashes($lineinfo['rest']);
                                    $share->notes[] = $note;
                                    $lineinfo = getLine();
                                    break;
                                case "NOTE":
...
function processMedia($mmcount, $mminfo, $persfamID, $eventID) {
    ...
    for ($mmctr = 1; $mmctr <= $mmcount; $mmctr++) {
        ...
        if (empty($mm['CHAN']) && !$tngimpcfg['chdate']) {
            $mm['CHAN'] = $today;
        }
...
function saveCitation($persfamID, $eventID, $cite) {
    ...
    $query = "INSERT INTO $citations_table (persfamID, gedcom, eventID, sourceID, description, citedate, citedatetr, citetext, page, quay, note, ordernum ) ";
    $query .= "VALUES('$persfamID', '$tree', '$eventID', '{$cite['sourceID']}', '{$cite['desc']}', '{$cite['DATE']}', '{$cite['DATETR']}', '{$cite['TEXT']}', '{$cite['PAGE']}', '{$cite['QUAY']}', '{$cite['NOTE']}', '0' )";
    $result = @tng_query($query) or die (_('Cannot execute query') . ": $query");
...
function getNoteRecord($noteID, $prevlevel) {
    ...
    if (tng_num_rows($result) && $savestate['del'] != "no") {
        ...
        $xresult = @tng_query($query) or die (_('Cannot execute query') . ": $query");
    } else {
    ...
    if ($privlen && substr($note, 0, $privlen) == $tngimpcfg['privnote']) {
        ...
        $nresult = @tng_query($query) or die (_('Cannot execute query') . ": $query");
    }
    ...
    for ($eventnum = 1; $eventnum <= $totevents; $eventnum++) {
        ...
        if (isset($custevents[$eventtype]) && $custevents[$eventtype]['keep']) {
            ...
            foreach ($eventptr['SHARES'] as $share) {
                $persfamID = $share->id;
                $query = "INSERT INTO $events_table (eventtypeID, persfamID, eventdate, eventdatetr, eventplace, age, agency, cause, addressID, parenttag, info, gedcom)  ";
                $query .= "VALUES('$eventtypeID', '$persfamID', \"" . $eventptr['DATE'] . "\", \"" . $eventptr['DATETR'] . "\", \"" . $eventptr['PLAC'] . "\", \"" . $eventptr['AGE'] . "\", \"" . $eventptr['AGNC'] . "\",
                 \"" . $eventptr['CAUS'] . "\", \"" . $eventptr['ADDR'] . "\",  \"" . $eventptr['parent'] . "\", \"$eventinfo\", '$tree')";
                $result = @tng_query($query) or die (_('Cannot execute query') . ": $query");
